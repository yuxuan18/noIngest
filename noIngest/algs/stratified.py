...
def run_stratified(data: Data, threshold: float, rate: float):
    all_detect_ids = []
    cost = 0
    ...
    estimate = Estimate(avg_est, avg_lb, avg_ub, count_est, count_ub, count_lb, rate, cost)
    estimate.save(f"results/uniform_{partition_str}.jsonl")

if __name__ == "__main__":
    data = Data("../../data/vehicle", partition=(41, 42))
    data.prep_features()
    run_stratified(data, 0.9, 0.1)
